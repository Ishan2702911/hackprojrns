...
export default function Dashboard({ onAnalysisUpdate, view = "pulse", customRepo }: DashboardProps) {
  ...
  const handleExportPDF = () => {
    if (!analysis?.documentation) return;
    const doc = analysis.documentation;
    const repo = selectedRepo?.full_name || "Repository";
    const date = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });

    const features = (doc.features || []).map((f: string) => `<li>${f}</li>`).join("");
    const fixes = (doc.fixes || []).map((f: string) => `<li>${f}</li>`).join("");

    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">
      <title>Release Notes — ${repo}</title>
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', system-ui, sans-serif; color: #1a1a2e; padding: 48px; max-width: 720px; margin: 0 auto; }
        .header { border-bottom: 3px solid #00b4d8; padding-bottom: 20px; margin-bottom: 32px; }
        .header h1 { font-size: 28px; font-weight: 800; color: #0077b6; letter-spacing: -0.5px; }
        .header p { color: #666; font-size: 13px; margin-top: 4px; }
        .badge { display: inline-block; background: #e0f7fa; color: #0077b6; font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 4px; margin-top: 8px; text-transform: uppercase; letter-spacing: 1px; }
        section { margin-bottom: 32px; }
        section h2 { font-size: 13px; font-weight: 800; text-transform: uppercase; letter-spacing: 2px; color: #666; margin-bottom: 12px; padding-bottom: 4px; border-bottom: 1px solid #eee; }
        ul { list-style: none; padding: 0; }
        ul li { font-size: 14px; line-height: 1.7; padding: 6px 0 6px 20px; position: relative; border-bottom: 1px solid #f5f5f5; }
        ul li::before { content: '▸'; position: absolute; left: 0; color: #00b4d8; font-size: 12px; top: 8px; }
        .fixes ul li::before { color: #e63946; }
      ...
      <section>
        <h2>✦ New Features</h2>
        <ul>${features || "<li>No features documented.</li>"}</ul>
      </section>
      <section class="fixes">
        <h2>✦ Bug Fixes &amp; Improvements</h2>
        <ul>${fixes || "<li>No fixes documented.</li>"}</ul>
      </section>
      <div class="footer">Generated by Artenis · Synthetic Architect v1.0</div>
    </body></html>`;
            ...
            <div className="lg:col-span-4 glass-panel rounded-xl border border-outline-variant/20 flex flex-col overflow-hidden">
              ...
              <div className="p-4">
                <button
                  ...
                  disabled={!analysis?.documentation}
                  className="w-full py-2 bg-secondary-container/20 text-secondary-fixed font-bold text-[10px] uppercase tracking-widest rounded hover:bg-secondary-container/30 transition-all active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed"
                >
